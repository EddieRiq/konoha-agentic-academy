...
class ManagedDistributionTests(unittest.TestCase):
    ...
    def test_pyproject_and_cli_version_are_aligned(self):
        ...
        self.assertIn('version = "4.1.1"', pyproject)
        self.assertIn('VERSION = "4.1.1"', version)

    def test_installer_default_release_tag_is_v4_1_1(self):
        # v4.1.1 release closure: the active installer default and its
        # help/example line must name the current installable release tag.
        source = INSTALLER.read_text(encoding="utf-8")
        self.assertIn('VERSION="v4.1.1"', source)
        self.assertIn("--version v4.1.1", source)
...
class CleanInstallSmokeTests(unittest.TestCase):
    def test_clean_install_smoke_passes(self):
        ...
        self.assertEqual(report["observed_version"], "4.1.1")
